...
import importlib
from utils.utils_custom.image_utils import save_image_grid, setup_snapshot_image_grid, save_watermark_diff_map
# --------------------------------- #
...
def training_loop(
    ...
    # --------------- W --------------- #
    water_config_path       = None,     # Path to the watermarking configuration file. 
    # --------------------------------- #
):
    # Initialize.
    ...
    # --------------- W --------------- #
    # Load and init the watermarking dictionnary from the json file configuration 
    ## 1. Loading step 
    ...
    ## 1.5 Total KIMG 
    if watermarking_dict_tmp.get('total_kimg'):
        total_kimg = watermarking_dict_tmp['total_kimg'] 
        print('KIMG:',kimg_per_tick)

    ## 2. Control in specific case ema_kimg and kimg_per_tick
    if watermarking_dict_tmp.get('ema_kimg'):
        ema_kimg = watermarking_dict_tmp['ema_kimg']          
    print('EMA_KIMG:',ema_kimg)
    
    if watermarking_dict_tmp.get('kimg_per_tick'):
        kimg_per_tick = watermarking_dict_tmp['kimg_per_tick'] 
    print('KIMG_PER_TICK:',kimg_per_tick)
    
    ## 3. Initialization of the watermarking dictionnary 
    if not watermarking_dict_tmp.get('batch_gpu'):
        watermarking_dict_tmp['batch_gpu'] = batch_gpu

    loss_kwargs.watermark_weight = watermarking_dict_tmp['watermark_weights']                # Watermarking weight default [mark_weight, imperceptibility_weight]
    loss_kwargs.G = G                                                                        # Generator full network architecture
    class_name = watermarking_dict_tmp['tools']
    import_file_path = 'NNWMethods.' + class_name.split('_')[0]
    lib = importlib.import_module(import_file_path)
     # Init the corresponding class methods for watermarking     
    for name, classe in lib.__dict__.items():
        if name == class_name : 
            loss_kwargs.tools = classe(device)                                    
    ...
    # Update watermarking_tmp dict with with new key and value
    watermarking_dict = loss_kwargs.tools.init(G, watermarking_dict_tmp, save=None)
    loss_kwargs.watermarking_dict = watermarking_dict
    # --------------------------------- #
    ...
    if rank == 0:
        ...
        if loss_kwargs.watermarking_dict:
            ...
            print('Initial image saved for watermarking diff map...')
        # --------------------------------- #
